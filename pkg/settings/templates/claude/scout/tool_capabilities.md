...
**Examples**:
1. **Map the Repo**: `gsc insights --db tiny-overview --fields keywords --limit 20 --format json`
2. **Find Latest Activity**: `gsc insights --db tiny-overview --fields dates | sort | tail -1` (Note: No `--format json` when piping to sort).
3. **Discover Keyword Clusters**: `gsc insights --db tiny-overview --fields keywords --filter "keywords in (*auth*,*token*)" --format json`
...
**Examples**:
1. **Find by Domain**: `gsc query --db tiny-overview --filter "keywords in (authentication)" --fields purpose,keywords --format json`
2. **Temporal Query**: `gsc query --db tiny-overview --filter "dates in (2026-03-15|*)" --fields purpose --format json` (Matches any date type for that day).
3. **Narrow with AND Logic**: `gsc query --filter "keywords in (blog)" --filter "dates in (2026-03-15|*)" --format json`
...
**Examples**:
1. **Discovery Grep (Token Saver)**: `gsc grep --summary --fields purpose,keywords --db tiny-overview --format json "buildIntelligence"` (Returns metadata only, no code snippets).
