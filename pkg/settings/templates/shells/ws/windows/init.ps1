...
# 3. Functions
function .ffp { gsc app ws ffp @args }
function .send { gsc app ws send @args }
function .help { Get-Content "$env:GSC_SCRIPTS_DIR/.gsc-welcome" }
function .map { gsc app ws map @args }

function .block {
    $target = gsc app ws block $args
...
function .goto {
    $selection = gsc app ws map --list | fzf --header "Jump to Workspace Block:" --reverse --height 40%
    if ($selection) {
