...
# Block-UUID: 93230ff5-79f1-4e3f-adbe-c8b4aabf975c
# Parent-UUID: ac4bc99c-5044-4d4b-91a7-428664b00dbd
# Version: 1.8.0
# Description: Updated .switch to use 'cd' instead of 'gsc app ws' to preserve the current shell session.
# Language: Zsh
...
# 3. Aliases
alias .ffp='gsc app ws ffp'
alias .send='gsc app ws send'
alias .help='cat ${GSC_SCRIPTS_DIR}/.gsc-welcome'
alias .map='gsc app ws map'

# 4. Block Navigation Function
.block() {
    local target=$(gsc app ws block "$@")
    if [[ -d "$target" ]]; then
        cd "$target"
    elif [[ -n "$target" ]]; then
        echo "$target"
...
.goto() {
    local selection=$(gsc app ws map --list | fzf --header "Jump to Workspace Block:" --reverse --height 40%)
    if [[ -n "$selection" ]]; then
