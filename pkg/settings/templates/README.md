...
This directory contains the launch templates for editors and terminals used by the `gsc app contract launch` command, as well as shell initialization scripts and help documentation.
...
Templates use a simple key-value pair where the key is the alias (used in `gsc app contract create --editor <alias>`) and the value is the command string.
...
```bash
gsc app contract create --code 123456 --description "Vim Workflow" --terminal iterm2-vim
```
