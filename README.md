...
### 4. Set Context
Define which database (manifest) you want to use. You can do this by creating a profile or using the `--db` flag.
```bash
# Create a profile for security analysis
gsc config context create security --db security-manifest --field risk_level
