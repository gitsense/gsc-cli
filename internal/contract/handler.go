 ...
 * Version: 1.27.0
 * Description: Simplified terminal intent handling to delegate shell spawning to 'gsc app ws'. Removed script generation and environment variable injection logic.
 * Language: Go
...
// If the terminal alias has a '-ws' suffix, it invokes 'gsc app ws <hash>' instead of a shell.
func handleTerminalIntent(meta *ContractMetadata, req LaunchRequest) (LaunchResult, error) {
	...
	if isWorkspace {
		// Workspace Mode: Invoke 'gsc app ws <hash>'
		// The template is expected to be "gsc app ws %s" or similar.
		...
		if hash == "" {
			...
			// Note: 'gsc app ws' does not currently support "latest" alias, 
