...
func (e *RipgrepEngine) buildArgs(options SearchOptions) []string {
	...
	// Add case sensitivity (three-way logic)
	// Priority: IgnoreCase > !CaseSensitive > default (case-sensitive)
	if options.IgnoreCase {
		args = append(args, "-i")
	} else if !options.CaseSensitive {
		args = append(args, "--smart-case")
	}
	// If both are false, ripgrep defaults to case-sensitive (no flag needed)
	...
	// Add multiline patterns (OR logic)
