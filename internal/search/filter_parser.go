...
func parseSingleFilter(filterStr string, fieldTypes map[string]string) (FilterCondition, error) {
	...
	// Detect standard operators using precise regex patterns
	// The patterns ensure that operators are matched as whole tokens, preventing
	// false positives like 'in' matching inside "pattern-matching".
	...
	// Order matters: longer operators must be checked first to avoid early matches
	operatorOrder := []string{"not in", "in", "!exists", "exists", "!=", ">=", "<=", "!~", "~", "=", ">", "<"}

	var op string
	var field, value string

	for _, candidateOp := range operatorOrder {
		pattern := operatorPatterns[candidateOp]
		re := regexp.MustCompile(pattern)
		if re.MatchString(filterStr) {
			op = candidateOp
			parts := re.Split(filterStr, 2)
			if len(parts) != 2 {
				// Handle exists/!exists which might not have a value after the operator
