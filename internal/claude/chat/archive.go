 ...
 * Block-UUID: 7b7aaa08-eba9-48c7-8e25-9e3a4aeb413f
 * Parent-UUID: ed198402-6fa9-4f00-83b0-4c4369cc0dd6
 * Version: 1.16.0
 ...
 * Language: Go
 * Created-at: 2026-05-08T02:42:35.043Z
 * Authors: GLM-4.7 (v1.8.0), claude-haiku-4-5-20251001 (v1.8.1), GLM-4.7 (v1.9.0), GLM-4.7 (v1.10.0), claude-haiku-4-5-20251001 (v1.11.0), GLM-4.7 (v1.12.0), GLM-4.7 (v1.12.1), GLM-4.7 (v1.13.1), GLM-4.7 (v1.13.2), Gemini 2.5 Flash (v1.14.0), claude-sonnet-4-6 (v1.15.0), GLM-4.7 (v1.16.0)
 */
...
// includeSystemMessages: if true, system messages are included in archive chunks
func SyncArchive(chatDir string, messages []db.Message, settings claude.Settings, includeSystemMessages bool) ([]claude.ArchiveFile, error) {
	...
	archiveFiles, err := writeArchiveChunks(messagesDir, archiveMessages, settings, includeSystemMessages)
	if err != nil {
	...
	if err := writeMessagesMap(messagesDir, cliOutputMessages, archiveFiles, contextFiles); err != nil {
...
func writeMessagesMap(dir string, cliOutputMessages []db.Message, archiveFiles []claude.ArchiveFile, contextFiles []context.ContextFile) error {
	...
	mapFile := claude.MapFile{
		...
		ContextFiles:   []claude.FileMeta{}, // Empty - context files moved to contexts.map (omitempty will omit this field)
...
func writeArchiveChunks(dir string, messages []db.Message, settings claude.Settings, includeSystemMessages bool) ([]claude.ArchiveFile, error) {
	...
	for i := 0; i < len(messages); i += settings.ChunkSize {
		...
		for _, m := range chunk {
			if m.Role == "system" && !includeSystemMessages {
				continue
			}
			content := ""
			if m.Message.Valid {
