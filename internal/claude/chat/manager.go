 ...
 * Block-UUID: 5186d61d-f18a-453e-b20a-111e3668e22c
 * Parent-UUID: 4ab90416-e6ef-4067-810b-43dd4c7250f7
 * Version: 1.62.0
 * Description: Updated executeCommand to use conditional prompt logic based on mode (analyze vs coding-assistant).
 * Language: Go
 * Created-at: 2026-05-08T03:25:30.789Z
 * Authors: claude-haiku-4-5-20251001 (v1.53.2), claude-haiku-4-5-20251001 (v1.53.3), GLM-4.7 (v1.54.0), GLM-4.7 (v1.54.1), GLM-4.7 (v1.54.2), GLM-4.7 (v1.55.0), Gemini 2.5 Flash (v1.56.0), GLM-4.7 (v1.57.0), GLM-4.7 (v1.57.1), GLM-4.7 (v1.58.0), GLM-4.7 (v1.59.0), GLM-4.7 (v1.60.0), GLM-4.7 (v1.61.0), GLM-4.7 (v1.62.0)
 */
...
func ExecuteChat(chatUUID string, assistantMessageID int64, userMessage string, format string, appendMsg bool, save bool, appendSave bool, model string, thinkingBudget int, mode string, noEvents bool) error {
	startTime := time.Now()
	...
	chatDB, metricsDB, _, chatDir, systemPromptPath, effectiveModel, storedSessionID, _, archiveSettings, err := setupAndPrepare(
		chatUUID, userMessage, assistantMessageID, appendMsg, appendSave, model, mode,
	)
	if err != nil {
		return err
	}
	...
	streamResult, err := executeCommand(
		chatDir, systemPromptPath, effectiveModel, storedSessionID, mode, thinkingBudget, format, archiveSettings, noEvents,
	)
	if err != nil {
		return err
	}
	...
	return finalizeAndSave(
		chatDB, metricsDB, chat, chatUUID, assistantMessageID, streamResult, effectiveModel, save, duration, noEvents,
	)
}
...
func setupAndPrepare(
	...
	*sql.DB,   // metricsDB
	...
	if mode == "analyze" {
		// Analyze Mode: Write expanded message files (no escaping)
		if err := writeExpandedMessages(chatDir, contextMessages); err != nil {
			chatDB.Close()
			metricsDB.Close()
			return nil, nil, "", "", "", "", "", nil, claude.Settings{}, fmt.Errorf("failed to write expanded messages: %w", err)
		}
	} else {
		// Coding Assistant Mode: Use archive-based protocol
		var historicalMessages []db.Message
		if len(contextMessages) > 0 {
			historicalMessages = contextMessages[:len(contextMessages)-1]
		}

		if _, err = SyncArchive(chatDir, historicalMessages, archiveSettings, false); err != nil {
			chatDB.Close()
			metricsDB.Close()
			return nil, nil, "", "", "", "", "", nil, claude.Settings{}, fmt.Errorf("failed to sync archive: %w", err)
		}

		// 8. Write User Message (Coding Assistant only)
		...
		// 9. Prepare CLAUDE.md (Coding Assistant only)
		if err := prepareClaudeMD(chatDir, gscHome); err != nil {
			logger.Warning("Failed to prepare CLAUDE.md", "error", err)
		}
	}

	// 10. Prepare System Prompt (Both modes)
	systemPromptPath := filepath.Join(chatDir, "messages", "system-prompt.md")
	defaultPrompt := "You are a helpful coding assistant."

	// Dynamic Template Resolution (map hyphens to underscores for filenames)
	templateFileName := strings.ReplaceAll(mode, "-", "_") + ".md"
	templatePath := filepath.Join(gscHome, settings.ClaudeTemplatesPath, templateFileName)
	
	if data, err := os.ReadFile(templatePath); err == nil {
		defaultPrompt = string(data)
		logger.Debug("Loaded template for mode", "mode", mode, "template", templateFileName)
	} else {
		logger.Warning("Template not found for mode, falling back to coding_assistant.md", "mode", mode, "error", err)
		fallbackPath := filepath.Join(gscHome, settings.ClaudeTemplatesPath, "coding_assistant.md")
	...
	prompt += "# Created-at: 2026-05-08T00:44:03.873Z\n"
	prompt += "# Authors: User (v1.0.0), GLM-4.7 (v1.0.1)\n"
	prompt += "\n"
	prompt += "\n"
...
func executeCommand(
	...
	var prompt string
	if mode == "analyze" {
		prompt = "Read `messages/messages.map` first.\n\n" +
			"This JSON file maps filenames to roles (e.g., `msg-001.md: system`).\n\n" +
			"Then, read each message file in order (msg-001.md, msg-002.md, etc.).\n\n" +
			"Interpret the content based on the role:\n" +
			"  - system: Your behavioral constraints (already set as system prompt)\n" +
			"  - assistant: Your specific analyzer instructions; read carefully\n" +
			"  - user: File content and/or the analysis trigger\n\n" +
			"Execute the analysis exactly as specified in the analyzer instructions.\n" +
			"Do not output anything until you have read all files."
	} else {
...
func finalizeAndSave(
	...
	// Output final response if in silent mode (batch processing)
	if noEvents {
		fmt.Fprint(os.Stdout, streamResult.FullResponse)
	} else {
		// Print provenance suffix for interactive mode
		fmt.Fprintf(os.Stdout, "\n\nAuthored by LLM %s at %s GMT\n", effectiveModel, time.Now().UTC().Format("Mon, 02 Jan 2006 15:04:05"))
	}
...
func replacePlaceholders(text, modelName, utcTime string) string {
	text = strings.ReplaceAll(text, "GLM-4.7", modelName)
	text = strings.ReplaceAll(text, "2026-05-08T00:44:03.873Z", utcTime)
	return text
}
...
// writeExpandedMessages writes individual message files and a map for analyze mode.
// This avoids JSON escaping issues by storing raw content in markdown files.
func writeExpandedMessages(chatDir string, messages []db.Message) error {
	dir := filepath.Join(chatDir, "messages")
	if err := os.MkdirAll(dir, DirPermissions); err != nil {
		return fmt.Errorf("failed to create messages directory: %w", err)
	}

	// 1. Write individual message files
	fileMap := make(map[string]string) // filename -> role
	for i, msg := range messages {
		// Zero-padded filename for sorting: msg-001.md, msg-002.md
		filename := fmt.Sprintf("msg-%03d.md", i+1)
		...
		fileMap[filename] = msg.Role
	}

	// 2. Write messages.map (JSON)
	mapPath := filepath.Join(dir, "messages.map")
	mapData, err := json.MarshalIndent(fileMap, "", "  ")
	if err != nil {
		return fmt.Errorf("failed to marshal messages.map: %w", err)
	}
	if err := os.WriteFile(mapPath, mapData, FilePermissions); err != nil {
