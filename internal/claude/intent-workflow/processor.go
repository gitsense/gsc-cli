 ...
 * Block-UUID: 8ba8603e-7dd5-4f0e-b7b6-ae8db05f5892
 * Parent-UUID: 89227332-2fdd-4f3b-8f8e-6047012ae86a
 * Version: 1.3.2
 * Description: Generic stream event processor for intent workflow sessions including JSONL event writing, reading, and session status reconstruction.
 * Language: Go
 * Created-at: 2026-04-26T18:14:58.246Z
 * Authors: claude-haiku-4-5-20251001 (v1.0.0), GLM-4.7 (v1.0.1), Gemini 3 Flash (v1.0.2), GLM-4.7 (v1.0.3), GLM-4.7 (v1.0.4), GLM-4.7 (v1.0.5), GLM-4.7 (v1.0.6), GLM-4.7 (v1.0.7), GLM-4.7 (v1.1.0), GLM-4.7 (v1.1.1), GLM-4.7 (v1.1.2), GLM-4.7 (v1.1.3), GLM-4.7 (v1.1.4), claude-haiku-4-5-20251001 (v1.1.5), GLM-4.7 (v1.2.0), GLM-4.7 (v1.3.0), GLM-4.7 (v1.3.1), GLM-4.7 (v1.3.2)
 */
...
func (ph *ProcessorHelper) GenerateStatusData(session *Session, currentTurn int) (*StatusData, error) {
	...
	if currentTurnState != nil {
		switch currentTurnState.TurnType {
		case "validation":
			phase = "validation"
		case "change", "resume-change":
			phase = "change"
		}
	}
	...
	if currentTurnState != nil {
		// Access TotalFound and Candidates from TurnResult.Discovery
		if currentTurnState.Result != nil && currentTurnState.Result.Discovery != nil {
			status.TotalFound = currentTurnState.Result.Discovery.TotalFound
			
			// Convert []Candidate to []Candidate for StatusData
			if len(currentTurnState.Result.Discovery.Candidates) > 0 {
				status.Candidates = make([]Candidate, len(currentTurnState.Result.Discovery.Candidates))
				for i, cand := range currentTurnState.Result.Discovery.Candidates {
					status.Candidates[i] = Candidate{
						...
						BrainMetadata: cand.BrainMetadata,
					}
				}
			}
		}
		...
		if currentTurnState.ErrorDetails != nil {
			status.ErrorDetails = currentTurnState.ErrorDetails
		}
	}
	...
	// Fallback: Use session-level error details if no turn error details exist
	if status.ErrorDetails == nil && session.ErrorDetails != nil {
		status.ErrorDetails = session.ErrorDetails
	}
