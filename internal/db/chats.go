 ...
 * Block-UUID: 6c9d73e0-d3d3-4e04-913d-fb21f3bfa783
 * Parent-UUID: 5189acd4-beba-4096-ad93-aeb7b9b4cbbc
 * Version: 1.26.2
 * Description: Fixed DeleteChatAndDescendants to correctly handle recursive soft deletes. Split the single SQL execution into two separate statements: one for messages (must run first while chats are active) and one for chats. This ensures the CTE scope is correct and all descendant messages are marked deleted before their parent chats. v1.26.2: Fixed timestamp format in DeleteChatAndDescendants to use ISO 8601 format consistent with the rest of the codebase, replacing SQLite's datetime('now') with Go's time.Now().UTC().Format.
 * Language: Go
 * Created-at: 2026-05-17T13:11:16.616Z
 * Authors: Gemini 3 Flash (v1.0.0), ..., GLM-4.7 (v1.23.0), GLM-4.7 (v1.24.0), GLM-4.7 (v1.25.0), GLM-4.7 (v1.26.0), GLM-4.7 (v1.26.1), GLM-4.7 (v1.26.2)
 */
...
func FormatContractMarkdown(data ContractMessageData) string {
	...
	// Informational Blurb
	sb.WriteString(fmt.Sprintf("\n**Note:** To retrieve your authorization code, run `gsc app contract info %s`\n", data.UUID))
...
// DeleteChatAndDescendants recursively soft deletes a chat and all its descendants.
// This is used by the --rebuild workflow to clean up a branch before re-importing.
// It marks both chats and their associated messages as deleted = 1.
func DeleteChatAndDescendants(db *sql.DB, refChatID int64) error {
	tx, err := db.Begin()
	if err != nil {
	...
	// Use consistent timestamp for both updates
	now := time.Now().UTC().Format("2006-01-02T15:04:05.000Z")

	// Step 1: Soft delete messages
	// This must run BEFORE chats are marked deleted, otherwise the CTE anchor (deleted=0) finds nothing.
	// We use a separate CTE here to ensure scope isolation.
	_, err = tx.Exec(`
	WITH RECURSIVE chat_tree AS (
		SELECT id FROM chats WHERE id = ? AND deleted = 0
		UNION ALL
		SELECT c.id FROM chats c
		INNER JOIN chat_tree ct ON c.parent_id = ct.id
		WHERE c.deleted = 0
	)
	UPDATE messages SET deleted = 1, updated_at = ? 
	WHERE chat_id IN (SELECT id FROM chat_tree)`, refChatID, now)
	
	if err != nil {
		return fmt.Errorf("failed to delete messages: %w", err)
	}

	// Step 2: Soft delete chats
	// Now that messages are handled, we can safely mark the chats as deleted.
	_, err = tx.Exec(`
	WITH RECURSIVE chat_tree AS (
		SELECT id FROM chats WHERE id = ? AND deleted = 0
		UNION ALL
	...
	UPDATE chats SET deleted = 1, updated_at = ? 
	WHERE id IN (SELECT id FROM chat_tree)`, refChatID, now)
	...
	logger.Info("Recursively soft deleted branch", "ref_chat_id", refChatID)
	return nil
}
