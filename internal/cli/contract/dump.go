 ...
 * Block-UUID: 51d64965-a209-4c08-a7bc-a9528118aa07
 * Parent-UUID: d4d094e8-3d83-400e-8eb4-b8fdf645ce40
 * Version: 1.2.0
 ...
 * Language: Go
 * Created-at: 2026-04-27T18:07:27.526Z
...
// dumpTreeCmd handles 'gsc app contract dump tree'
...
// dumpMergedCmd handles 'gsc app contract dump merged'
...
// dumpMappedCmd handles 'gsc app contract dump mapped'
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if contractDumpValidate {
			if !result.Exists {
				...
				fmt.Printf("  Run 'gsc app contract dump mapped --message-id %d' to initialize.\n", contractDumpMessageID)
				return nil
			}
...
// dumpPruneCmd handles 'gsc app contract dump prune'
...
func init() {
	...
	dumpContractCmd.PersistentFlags().StringVar(&contractDumpUUID, "contract", "", "Contract UUID (optional if in workdir)")
