 ...
 * Block-UUID: cb23e448-37c2-4db9-8aa3-8cbf7ce507cd
 * Parent-UUID: 57f701d0-afb3-404d-b423-d8e6919cd77a
 * Version: 1.0.4
 * Description: CLI commands for managing contract lifecycle: status, list, cancel, renew, complete, delete, and info.
 * Language: Go
 * Created-at: 2026-03-26T20:43:43.495Z
 * Authors: Gemini 3 Flash (v1.0.0), ..., GLM-4.7 (v1.0.2), GLM-4.7 (v1.0.3), GLM-4.7 (v1.0.4)
 */
...
// statusContractCmd handles 'gsc app contract status'
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "no active contract") {
				...
				fmt.Println("  gsc app contract create --code <6-digit-code> --description \"Purpose of contract\"")
				return nil
			}
			return err
		}
...
// listContractCmd handles 'gsc app contract list'
...
// cancelContractCmd handles 'gsc app contract cancel [uuid]'
...
// renewContractCmd handles 'gsc app contract renew [uuid]'
...
// completeContractCmd handles 'gsc app contract complete [uuid]'
...
// deleteContractCmd handles 'gsc app contract delete [uuid]'
...
// infoContractCmd handles 'gsc app contract info [uuid]'
