 ...
 * Block-UUID: 5880d3a0-0947-49d8-8c83-b021692c8198
 * Parent-UUID: 7c2f8bba-461a-4d33-b49c-8f3b0a1e9c27
 * Version: 1.1.0
 * Description: CLI commands for managing working directories within a contract (add, list, remove, set-primary).
 * Language: Go
 * Created-at: 2026-04-27T18:06:51.060Z
...
// addWorkdirCmd handles 'gsc app contract add-workdir'
...
the directory's basename will be used.`,
  ...
  gsc app contract add-workdir --path ../shared-utils --name "shared-utils"

  # Add a directory (defaults name to folder name)
  gsc app contract add-workdir --path /opt/libs/api-gateway`,
...
// listWorkdirsCmd handles 'gsc app contract list-workdirs'
...
// removeWorkdirCmd handles 'gsc app contract remove-workdir'
...
The primary workdir cannot be removed. Use 'set-primary-workdir' to change the primary first.`,
	Example: `  gsc app contract remove-workdir --name "shared-utils"`,
	RunE: func(cmd *cobra.Command, args []string) error {
		cmd.SilenceUsage = true
...
// setPrimaryWorkdirCmd handles 'gsc app contract set-primary-workdir'
...
of another active contract to prevent conflicts.`,
	Example: `  gsc app contract set-primary-workdir --name "user-service"`,
	RunE: func(cmd *cobra.Command, args []string) error {
		cmd.SilenceUsage = true
...
func init() {
	...
	addWorkdirCmd.Flags().StringVar(&dirUUID, "contract", "", "Contract UUID (optional, defaults to current directory)")
	addWorkdirCmd.MarkFlagRequired("path")

	// List Workdirs Flags
	listWorkdirsCmd.Flags().StringVarP(&dirFormat, "format", "f", "human", "Output format (human, json)")
	listWorkdirsCmd.Flags().StringVar(&dirUUID, "contract", "", "Contract UUID (optional, defaults to current directory)")

	// Remove Workdir Flags
	removeWorkdirCmd.Flags().StringVar(&dirName, "name", "", "Name of the workdir to remove (required)")
	removeWorkdirCmd.Flags().StringVar(&dirUUID, "contract", "", "Contract UUID (optional, defaults to current directory)")
	...
	setPrimaryWorkdirCmd.Flags().StringVar(&dirUUID, "contract", "", "Contract UUID (optional, defaults to current directory)")
