...
func findContractUUIDByWorkdir() (string, error) {
	...
	if len(matches) == 0 {
		if len(expiredMatches) > 0 {
			return "", fmt.Errorf("no active contract found. Found expired contract(s): %s. Run 'gsc app contract renew <contract uuid>' to reactivate.", strings.Join(expiredMatches, ", "))
		}
