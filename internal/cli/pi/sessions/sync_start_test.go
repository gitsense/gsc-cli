 ...
 * Version: 1.1.0
 * Description: Verifies foreground watcher wiring, inherited flags, default paths, cancellation, and detach mode daemonization.
 * Language: Go
...
func TestSyncStartDetachedModeCallsDaemonize(t *testing.T) {
	sessionsDir := t.TempDir()
	dbPath := filepath.Join(t.TempDir(), "sessions.sqlite3")
	daemonizeCalled := false
	dependencies := syncStartDependencies{
		watch: func(ctx context.Context, options pisessions.WatchOptions) error {
			t.Fatal("watch should not be called in daemonize path")
			return nil
		},
		notifyContext: func(parent context.Context, signals ...os.Signal) (context.Context, context.CancelFunc) {
			t.Fatal("notifyContext should not be called in daemonize path")
			return parent, func() {}
		},
		daemonize: func(cmd *cobra.Command, gotSessionsDir, gotDBPath string) error {
			daemonizeCalled = true
			absSessionsDir, _ := filepath.Abs(sessionsDir)
			absDBPath, _ := filepath.Abs(dbPath)
			if gotSessionsDir != absSessionsDir || gotDBPath != absDBPath {
				t.Fatalf("daemonize args = (%q, %q), want (%q, %q)", gotSessionsDir, gotDBPath, absSessionsDir, absDBPath)
			}
			return nil
		},
	}
	...
	cmd.SetOut(&output)
	cmd.SetErr(&output)
	cmd.SetArgs([]string{"start", "--detach", "--sessions-dir", sessionsDir, "--db", dbPath})
	err := cmd.ExecuteContext(context.Background())
	if err != nil {
		t.Fatalf("sync start --detach: %v", err)
	}
	if !daemonizeCalled {
		t.Fatal("daemonize was not called")
	}
}
