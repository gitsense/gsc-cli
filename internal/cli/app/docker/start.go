 ...
 * Block-UUID: f5e9ea6b-c0f1-4461-a383-10218e55bdd4
 * Parent-UUID: 093efe63-b861-49f4-8050-6a43517021a7
 * Version: 1.15.0
 * Description: Fixed all command references from 'gsc docker' to 'gsc app docker' for consistency with the CLI command structure.
 * Language: Go
 * Created-at: 2026-05-13T00:42:00.000Z
 * Authors: Gemini 3 Flash (v1.0.0), GLM-4.7 (v1.1.0), GLM-4.7 (v1.2.0), GLM-4.7 (v1.3.0), GLM-4.7 (v1.4.0), Gemini 3 Flash (v1.6.0), Gemini 3 Flash (v1.7.0), Gemini 3 Flash (v1.8.0), Gemini 3 Flash (v1.9.0), GLM-4.7 (v1.10.0), GLM-4.7 (v1.10.1), GLM-4.7 (v1.11.0), GLM-4.7 (v1.12.0), GLM-4.7 (v1.13.0), GLM-4.7 (v1.14.0), GLM-4.7 (v1.15.0)
 */
...
mounts the specified repository directory, and launches the application.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err == nil && running {
			return fmt.Errorf("container '%s' is already running. Stop it first with: gsc app docker stop", containerName)
		}
		...
		fmt.Println("   gsc app docker configure --repos-dir /path/to/your/code")
		...
		fmt.Printf("\n✓ Container '%s' started successfully on port %s\n", containerName, port)
		fmt.Printf("   Access it at: http://localhost:%s\n", port)
		fmt.Printf("   View logs with: gsc app docker logs\n")
		...
		if _, err := os.Stat(persistentEnvPath); os.IsNotExist(err) {
			...
			fmt.Println("      gsc app docker env link <path-to-your-file>")
			fmt.Println("")
			fmt.Println("   2. Create a new master configuration file:")
			fmt.Println("      gsc app docker env init")
