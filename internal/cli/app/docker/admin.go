 ...
 * Block-UUID: f2399d39-3e68-485c-a68f-b591ca27b03a
 * Parent-UUID: f170bd65-1aed-4f15-8b61-da1e14c5e310
 * Version: 2.2.0
 * Description: Fixed command references from 'gsc docker' to 'gsc app docker' for consistency with the CLI command structure.
 * Language: Go
 * Created-at: 2026-05-13T01:00:00.000Z
 * Authors: Gemini 3 Flash (v1.0.0), Gemini 3 Flash (v2.0.0), GLM-4.7 (v2.1.0), GLM-4.7 (v2.2.0)
 */
...
and LLM settings for the running GitSense Chat container.`,
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		...
		if dctx == nil {
			logger.Fatal("No active Docker context found. Run 'gsc app docker start' first")
		}
		...
		if !running {
			logger.Fatal("Container is not running. Start it with 'gsc app docker start'")
		}
	},
}
...
func promptRestartIfNeeded() error {
	...
	if confirm {
		...
		fmt.Println("Remember to restart manually with 'gsc app docker restart'")
	}
	return nil
}
