...
func Watch(ctx context.Context, options WatchOptions) error {
	...
	defer source.Close()
	logger := newSyncLogger(options.LogPath, options.DebugLogPath)
	defer logger.Close()
	reconcile := func(reconcileCtx context.Context) error {
		logger.LogDebug("starting reconciliation")
		result, err := Sync(reconcileCtx, SyncOptions{SessionsDir: sessionsDir, DBPath: options.DBPath, Logger: logger})
		if err != nil {
			logger.LogError("reconciliation failed: " + err.Error())
			return err
		}
		for _, syncErr := range result.Errors {
			logger.LogError(fmt.Sprintf("%s: %s", syncErr.Path, syncErr.Error))
		}
		if result.SessionsImported > 0 || result.MessagesImported > 0 {
			logger.LogInfo(fmt.Sprintf("synced %d sessions, %d messages", result.SessionsImported, result.MessagesImported))
		}
		logger.LogDebug(fmt.Sprintf("reconciliation complete: %d files scanned, %d sessions imported, %d messages", result.FilesScanned, result.SessionsImported, result.MessagesImported))
		return nil
	}
	return runWatchLoop(ctx, options.DebounceInterval, options.ReconcileInterval, source, reconcile, logger)
}

func runWatchLoop(ctx context.Context, debounceInterval time.Duration, reconcileInterval time.Duration, source watchEventSource, reconcile func(context.Context) error, logger *syncLogger) error {
	if err := reconcile(ctx); err != nil {
		logger.LogError(fmt.Sprintf("stopping sync: initial reconciliation failed: %s", err.Error()))
	...
	reconcilePending := func() error {
		...
		if err := reconcile(ctx); err != nil {
			...
			if consecutiveErrors >= maxConsecutiveErrors {
				logger.LogError(fmt.Sprintf("stopping sync: too many consecutive reconciliation errors (%d): %s", consecutiveErrors, err.Error()))
				return fmt.Errorf("too many consecutive reconciliation errors (%d): %w", consecutiveErrors, err)
			}
			logger.LogError(fmt.Sprintf("reconciliation error (will retry, %d/%d consecutive): %s", consecutiveErrors, maxConsecutiveErrors, err.Error()))
			return nil
		}
		consecutiveErrors = 0
		clear(pending)
...
// syncLogger writes sync errors and info to a log file for status display.
type syncLogger struct {
	file      *os.File
	debugFile *os.File
}

func newSyncLogger(logPath, debugLogPath string) *syncLogger {
	logger := &syncLogger{}
	...
	if debugLogPath != "" {
		if err := os.MkdirAll(filepath.Dir(debugLogPath), 0755); err == nil {
			if file, err := os.OpenFile(debugLogPath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644); err == nil {
...
func (l *syncLogger) LogError(msg string) {
	l.write(l.file, "ERROR", msg)
}

func (l *syncLogger) LogInfo(msg string) {
	l.write(l.file, "INFO", msg)
}

func (l *syncLogger) LogDebug(msg string) {
	l.write(l.debugFile, "DEBUG", msg)
}

func (l *syncLogger) write(file *os.File, level, msg string) {
	if file == nil {
		return
	}
