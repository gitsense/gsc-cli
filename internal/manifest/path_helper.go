 ...
 * Block-UUID: b73f7303-f614-4aa5-9bc7-26fcc442f312
 * Parent-UUID: 6cf4f31f-117f-4e12-81bd-e2ef4d0833fc
 * Version: 1.8.0
 ...
 * Authors: GLM-4.7 (v1.0.0), Claude Haiku 4.5 (v1.1.0), GLM-4.7 (v1.2.0), GLM-4.7 (v1.3.0), GLM-4.7 (v1.4.0), Gemini 3 Flash (v1.5.0), Gemini 3 Flash (v1.6.0), claude-haiku-4-5-20251001 (v1.7.0), GLM-4.7 (v1.8.0)
 */
...
// ResolveManifestsDir constructs the absolute path to the .gitsense/manifests directory.
// This directory is used to store pre-packaged manifest files for easy import.
func ResolveManifestsDir() (string, error) {
	root, err := git.FindProjectRoot()
	if err != nil {
		return "", fmt.Errorf("failed to find project root: %w", err)
	}

	manifestsDir := filepath.Join(root, settings.GitSenseDir, "manifests")
	return manifestsDir, nil
}

// ResolveManifestPath attempts to find a manifest file by name in the .gitsense/manifests directory.
// It appends .json to the name if not present and checks if the file exists.
func ResolveManifestPath(name string) (string, error) {
	...
	manifestPath := filepath.Join(manifestsDir, filename)

	// Check if the file actually exists
	if _, err := os.Stat(manifestPath); os.IsNotExist(err) {
		return "", fmt.Errorf("manifest file not found in .gitsense/manifests: %s", filename)
	}
